<template>
  <div class="recognition-section">
    <div class="results-header">
      <h1 class="results-title">Сдача инструментов - Распознавание</h1>
...
const recognitionData = computed(() => props.session.handover?.predict)

const imageData = computed(() => {
  return recognitionStore.recognitionData?.image || props.session.handover?.image || ''
})
...
const loadImageFromData = async () => {
  if (!imageData.value) {
    console.warn('No image data available for handover')
    return
  }
  ...
  try {
    ...
    img.onload = async () => {
      console.log('Handover image loaded successfully:', img.width, 'x', img.height)
      ...
      console.log('Handover stage config:', stageConfig)
      console.log('Handover image config:', imageConfig)
      ...
      // Force redraw
      if (imageLayer.value) {
        console.log('Force redraw handover image layer')
        imageLayer.value.getNode().batchDraw()
      }
    }
    
    img.onerror = (error) => {
      console.error('Failed to load handover image:', error)
      notifications.error('Не удалось загрузить изображение для сдачи. Проверьте формат файла и попробуйте ещё раз.', {
        title: 'Ошибка загрузки изображения'
      })
    }
    
    console.log('Loading handover image from data')
    img.src = imageData.value
  } catch (error) {
    console.error('Error loading handover image:', error)
    notifications.error('Произошла ошибка при загрузке изображения для сдачи.', {
      title: 'Ошибка загрузки'
    })
  }
}

// Helper methods (reusing logic from HandoutRecognition)
...
// Konva helper methods (reusing from HandoutRecognition with slight styling differences)
const getDetectionBoxConfig = (detection: UIDetection, index: number) => {
...
// Event handlers (simplified versions similar to HandoutRecognition)
...
const addManualAnnotation = (className: string) => {
  ...
  notifications.info(
    `Нарисуйте прямоугольник на изображении, чтобы отметить расположение инструмента при сдаче.`,
    { 
...
// Stage event handlers (identical to HandoutRecognition)
...
const handleStageMouseUp = (e: any) => {
  if (isDrawing.value && newAnnotation.value && drawingClass.value) {
    ...
    const newDetection: Detection = {
      detection_id: `manual-handover-${Date.now()}`,
...
const savePositionChanges = (detection: Detection) => {
  // Implementation similar to HandoutRecognition
  isEditingPosition.value = false
  editingDetectionId.value = null
  hasUnsavedChanges.value = false
...
const handleDetectionDragEnd = (detection: UIDetection) => {
  if (editingDetectionId.value === detection.detection_id) {
    hasUnsavedChanges.value = true
  }
}

const handleDetectionDragMove = (detection: UIDetection, e: any) => {
  if (editingDetectionId.value === detection.detection_id) {
...
const resizingDetection = ref<UIDetection | null>(null)
...
const handleResizeStart = (detection: UIDetection, handleType: string, e: any) => {
...
const handleResizeDrag = (detection: UIDetection, e: any) => {
...
const handleResizeEnd = (detection: UIDetection) => {
...
const saveAnnotations = async () => {
  ...
  try {
    const annotations: Annotation[] = allDetections.value.map((detection) => ({
      ...
      box: detection.box,
      source: detection.detection_id?.startsWith('manual-handover-') ? 'manual' : 
               detection.detection_id?.startsWith('edited-') ? 'edited' : 'model',
      from_detection_id: detection.detection_id?.startsWith('manual-handover-') ? undefined : detection.detection_id
    }))

    const result = await sessionStore.handoverAdjust(props.session.id, {
    ...
    if (result.passed) {
      notifications.success(
        'Разметка сдачи завершена! Переходим к сравнению с выдачей.',
        { 
      ...
      // Emit complete event to trigger session refresh and show diff
      emit('complete')
    } else {
      ...
      notifications.error(
        ...
        { 
          title: 'Ошибки в разметке сдачи',
          duration: 0
        }
      )
      // Don't emit complete event if validation failed
    }
  } catch (error: any) {
    console.error('Error saving handover annotations:', error)
    // notifications.error(error.message || 'Не удалось сохранить разметку сдачи. Проверьте подключение к интернету и попробуйте ещё раз.', {
